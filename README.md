...
<p align="center">
  ...
  <a href="#-jargon-decoder">🗣 Jargon decoder</a> &nbsp;·&nbsp;
  <a href="#-faq">❓ FAQ</a>
</p>
...
| | 📚 bookshelf-in-a-box | ☁️ Kindle / Kobo cloud | 🖥 Calibre on the desktop | 🔧 Raw Calibre-Web + Docker |
|---|---|---|---|---|
| **Your books stay yours** | ✅ On your disk, forever | ⚠️ Tied to an account; titles can vanish | ✅ | ✅ |
| **Read on any device** | ✅ Phone, tablet, e-reader, browser | ⚠️ Their devices & apps only | ❌ Plug in a USB cable like it's 2009 | ✅ |
| **Setup effort** | 😌 One command, friendly questions | 😌 Easy (that's the trap) | 😐 Fine until you want it on your phone | 🥵 YAML, volumes, PUID/PGID, good luck |
| **Auto-import & convert** | ✅ Drop a file in a folder, done | — | ❌ Manual clicking | ⚠️ Some assembly required |
| **Monthly cost** | **£0 / $0** | 💸 Often a subscription | £0 | £0 |
| **Backups, themes, stats, remote access** | ✅ Built-in helpers | — | ❌ | ❌ DIY |

*(This project **is** Calibre-Web underneath — the last column is what setting it up by hand feels like. We just did the hard parts for you.)*
...
Everything lives inside this one folder — delete the folder and it's like it never happened; back up the folder and you can never lose your library:

```text
bookshelf-in-a-box/
├── setup.sh / setup.ps1     ← the one command you ran (checks, asks, launches)
├── .env                     ← your settings (port, timezone) — plain text, edit freely
├── docker-compose.yml       ← tells Docker what to run
├── docker-compose.*.yml     ← optional add-ons (backups, HTTPS, Tailscale, themes…)
├── bin/bookshelf            ← your friendly remote control (status, doctor, backup…)
├── data/
│   ├── ingest/              ← 📥 DROP BOOKS HERE — they get whisked away automatically
│   ├── library/             ← 📚 your actual library — THIS is the folder to back up
│   └── config/              ← the app's own settings & database
├── backups/                 ← timestamped snapshots land here
└── web/themes/              ← CSS themes for restyling the app
```

Two folders to remember: **`data/ingest`** is the in-tray, **`data/library`** is the vault. Everything else is machinery.

<details>
<summary><strong>🔍 What did the setup script actually do?</strong> (for the curious — nothing is hidden)</summary>

1. **Checked your machine** — Docker installed? Docker running? Ports free? Enough disk?
2. **Created the `data/` folders** and set their permissions so the container can write to them.
3. **Asked two questions** (timezone, port) and wrote the answers into `.env`. That's the whole config.
4. **Started the container** with `docker compose up -d` — the same command you'd type by hand.
5. **Waited for the app to come up healthy**, then printed your addresses and opened the welcome page.

No accounts created, nothing phoned home, nothing installed outside this folder (Docker images aside). Re-running it is always safe — it never overwrites an existing `.env` or your books.
</details>
...
<details>
<summary><strong><code>./setup.sh</code> prints <code>./.env: line 36: 3: command not found</code></strong></summary>

Your `.env` was generated by an older version that wrote the backup schedule without quotes (`BACKUP_CRON=0 3 * * *`). When the setup script loads the file, the shell misreads that line as a command. It's cosmetic, but fix it by wrapping the two cron values in quotes in `.env`:
...
New installs (and a fresh `.env` from the current `.env.example`) already have the quotes.
</details>
...
Self-hosting has its own dialect. Here's every term this README uses, in plain English:

| Term | What it actually means |
|---|---|
| **Self-hosted** | The app runs on *your* computer instead of a company's server. You're the cloud now. ☁️→🏠 |
| **Docker** | An app that runs other apps in tidy, sealed boxes ("containers") so they can't mess up your computer — and one `docker compose up` starts everything identically on any machine. |
| **Container** | One of those sealed boxes. Ours contains the entire ebook server. Delete it and your books are untouched (they live in `data/`, outside the box). |
| **`.env` file** | A plain-text settings file: `PORT=8083`, one setting per line. The only "configuration" this project has. |
| **OPDS** | "RSS for ebooks" — a standard catalog format that reading apps understand, which is why one URL works in KOReader, Moon+ Reader, Yomu, Panels… |
| **Ingest** | Fancy word for the in-tray. Files dropped in `data/ingest` are automatically converted, tagged, and filed into the library. |
| **Tailscale** | A free app that puts your devices on a private, encrypted mini-network, so your phone can reach your home server from anywhere — without exposing anything to the internet. |
| **Port** | A numbered "door" on a computer. Your library answers on door `8083`; `http://localhost:8083` means "door 8083 on this machine." |
| **Port forwarding** | Opening one of those doors to the entire internet. **Don't do this** — use Tailscale instead. |
| **Reverse proxy (Caddy)** | A doorman that sits in front of the app and adds niceties like HTTPS. Optional here. |
| **SMTP** | The protocol for *sending* email. Needed only for Send-to-Kindle, which literally emails books to your device. |
| **PUID / PGID** | The user account (by number) the container pretends to be, so the files it writes belong to *you*, not to root. The setup script fills these in for you. |
| **Cron** | A scheduler that runs at set times, e.g. `0 3 * * *` = "every day at 03:00." Used by the nightly backups. |
